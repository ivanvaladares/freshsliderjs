    ...
    $.fn.freshslider = function (options) {
        ...
        var onMoveStart = function (e) {
            ...
            if (isSingle) {
                ...
                if (isRight) {
                    var valR = realX / realWidth;
                    if (valR < values[0]) {
                        valR = values[0];
                    }
                    values[1] = valR;
                } else {
                    var valL = realX / realWidth;
                    if (valL > values[1]) {
                        valL = values[1];
                    }
                    values[0] = valL;
        ...
        $(me).find(".fscaret.fss-left").on("mousedown touchstart", function () { isRight = false });
        $(me).find(".fscaret.fss-right").on("mousedown touchstart", function () { isRight = true });
        $(me).find(".fscaret").on("mousedown touchstart", function () { isDown = true });
        $(document).on("mousemove touchmove", onMoveStart);
